      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="md:hidden absolute top-full left-0 w-full glass-panel border-t border-white/5 py-4 px-4 flex flex-col gap-4 shadow-2xl"
          >
            {navLinks.map((link) => (
              <a
                key={link.name}
                ...
                className="text-lg font-medium text-muted-foreground hover:text-primary px-4 py-2 rounded-lg hover:bg-white/5 transition-all"
              >
                {link.name}
              </a>
            ))}
